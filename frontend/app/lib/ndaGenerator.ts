import { NDAFormData } from "./ndaTypes";
...
function mndaTermLabel(data: NDAFormData): string {
  if (data.mndaTermType === "expires") {
    const yrs = data.mndaTermYears || "1";
    return `${yrs} year${yrs === "1" ? "" : "s"} from Effective Date`;
  }
...
function confidentialityTermLabel(data: NDAFormData): string {
  if (data.confidentialityTermType === "years") {
    const yrs = data.confidentialityTermYears || "1";
    return `${yrs} year${yrs === "1" ? "" : "s"} from Effective Date`;
  }
...
export function generateNDAMarkdown(data: NDAFormData): string {
  ...
  const replacements: Record<string, string> = {
    ...
    "{{PARTY1_NAME}}": p1.printName || " ",
    "{{PARTY1_TITLE}}": p1.title || " ",
    "{{PARTY1_COMPANY}}": p1.company || " ",
    "{{PARTY1_ADDRESS}}": p1.noticeAddress || " ",
    "{{PARTY2_NAME}}": p2.printName || " ",
    "{{PARTY2_TITLE}}": p2.title || " ",
    "{{PARTY2_COMPANY}}": p2.company || " ",
    "{{PARTY2_ADDRESS}}": p2.noticeAddress || " ",
  };

  let coverPage = COVER_PAGE_TEMPLATE;
  let standardTerms = STANDARD_TERMS_TEMPLATE;

  for (const [key, value] of Object.entries(replacements)) {
    coverPage = coverPage.replaceAll(key, value);
    standardTerms = standardTerms.replaceAll(key, value);
  }
...
export function downloadMarkdown(data: NDAFormData): void {
  ...
  a.download = `Mutual-NDA_${party1}_${party2}.md`.replace(/\s+/g, "-");
  a.click();
